...
public sealed record StartMakingOrderCommand(Guid OrderId) : ICommand, ITenantRoleRequest
{
    ...
    internal sealed class Validator : AbstractValidator<StartMakingOrderCommand>
    {
        public Validator()
        {
    ...
    [SuppressMessage(
        ...
        "CA1812: Avoid uninstantiated internal classes",
        Justification = "Used via mediation")]
    internal sealed class Handler : IRequestHandler<StartMakingOrderCommand, CommandResult>
    {
        ...
        public async Task<CommandResult> Handle(
            StartMakingOrderCommand request,
            CancellationToken cancellationToken)
        {
            ...
            var result = order.StartMaking(correlationId);
            if (!result.IsSuccess)
            {
            ...
            this.logger.LogInformation("Order {OrderId} started being made", request.OrderId);
