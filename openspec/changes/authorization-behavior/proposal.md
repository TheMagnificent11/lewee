...
- Add role assignment to `Lewee.Auth.Domain`: a `TenantMembership` can be assigned zero, one, or many roles (each a simple string code, e.g. a Pizzeria-defined "Manager", "Store Worker", or "Delivery Driver"). Roles are a fixed, application-defined set - they are not owned or defined per `Tenant` - so no tenant-scoped role catalog, uniqueness check, or cross-tenant validation is needed.
...
### New Capabilities
- `auth/role-management`: Defines how roles (a fixed, application-defined set of string codes) are assigned to/removed from a `User`'s `TenantMembership`, with a membership able to hold multiple roles concurrently.
...
- None. `auth/tenant-management`, `auth/user-provisioning`, and `auth/data-persistence` (proposed by `openspec/changes/multi-tenancy-support`, not yet archived into `openspec/specs/`) are extended by, but not changed in their existing requirements by, this plan - `TenantMembership`'s existing creation/removal/idempotency behavior is unaffected; `User.IsSiteAdministrator` and roles are additive.

## Impact

- **Affected code (framework, `src/`)**:
  - `src/Lewee.Auth.Domain`: `User` gains `IsSiteAdministrator`; `TenantMembership` gains role assignment(s) (a simple string code per assignment, no `Role` catalog entity); new domain events (e.g. role assigned/removed).
  - `src/Lewee.Auth.Infrastructure.Data`: EF configuration/migration for `IsSiteAdministrator` and role assignments, plus the new authorization lookup read model/table and its maintenance.
