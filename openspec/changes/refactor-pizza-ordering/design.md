...
**Decision**: Add `PizzasPickedUp`/`PizzasDelivered` command handlers (wrapping the existing `Order.PickedUp()`/`Order.Delivered()` domain methods) and a `GetOrdersQuery` supporting a status filter (current vs past), all implementing `ITenantRoleRequest` with `Store Staff` and `Store Manager` as satisfying roles. Menu management commands (`AddPizza`, `EditPizza`, `RemovePizza` on `Menu`/`Pizza`) implement `ITenantRoleRequest` with only `Store Manager` as the satisfying role.
