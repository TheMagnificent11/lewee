...
- [Splitting `Pizzeria.Configuration` migration/seeding logic across two `DbContext`s increases startup complexity and ordering risk (auth migration/seed must precede the `sto.Users`-dropping migration)] → Mitigate by sequencing `MigrateDatabaseAsync<AuthDbContext>` before `MigrateDatabaseAsync<StoreDbContext>` in `Pizzeria.Configuration`'s `Program.cs`/`PizzeriaConfigurationService`, and by adding a test/verification step confirming both contexts migrate cleanly in order against a fresh database.
...
4. Create `Lewee.Auth.Api` with `CreateUserEndpoint`, moved/renamed from `Pizzeria.Store.Api.Customers.CreateCustomerEndpoint`; add project to `lewee.slnx`; host it in `Pizzeria.Auth.Api`.
...
9. Add `Pizzeria.Bff`, route auth and store requests to their respective APIs, and update `Pizzeria.Store.Web`'s `TokenValidatedContextExtensions`/`IBffApiClient` and SSE client to call the BFF.
