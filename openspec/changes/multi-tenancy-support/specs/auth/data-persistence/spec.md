...
`Tenant` and `User` data SHALL be persisted via a database context whose default schema is `auth`, separate from any application-specific schema (such as `sto`). This context SHALL NOT contain application-specific aggregates (e.g. `Order`, `Pizza`).
...
- **THEN** the resulting `Tenants` and `Users` tables SHALL exist in the `auth` schema

### Requirement: Existing user data is migrated from the sto schema to the auth schema
Existing rows in the `sto.Users` table SHALL be migrated into the new `auth.Users` table (associated with a `Tenant` record) as part of the rollout of this change, and the `sto.Users` table SHALL be removed once migration completes.
...
- **THEN** each existing user SHALL have a corresponding row in `auth.Users`, associated with a `Tenant`, and no data SHALL be lost
